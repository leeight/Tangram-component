 ...
 */
<<<<<<< HEAD
baidu.ui.behavior.statable = function() {
    // 行为初始化
};

// 保存实例中所有的状态，格式：group+elementId : {stateA : 1, stateB : 1}
baidu.ui.behavior.statable._states = {};

// 所有可用的状态，调用者通过addState添加
baidu.ui.behavior.statable._allStates = ['hover', 'press', 'disabled'];

baidu.ui.behavior.statable._allEventsName = ['mouseover', 'mouseout', 'mousedown', 'mouseup'];

baidu.ui.behavior.statable._eventsHandler = {
 ...
 * @return {string} 元素属性字符串.
 */
baidu.ui.behavior.statable._getStateHandlerString = function(group, key) {
    var me = this,
        i = 0,
        len = me._allEventsName.length,
        ret = [],
    ...
    for (; i < len; i++) {
        eventType = me._allEventsName[i];
        ret[i] = 'on' + eventType + '=\"' + me.getCallRef() + 
            "._fireEvent('" + eventType + "', '" + group + "', '" + key + "', event)\"";
    }
 ...
 * @param {DOMEvent} e DOM原始事件.
 */
baidu.ui.behavior.statable._fireEvent = function(eventType, group, key, e) {
    var me = this,
        id = me.getId(group + key);
    if (me._eventsHandler[eventType].call(me, id, group)) {
        me.dispatchEvent(eventType, {
 ...
 * 让一个元素处于disable状态
 */
baidu.ui.behavior.statable.disable = function(element, group) {
    var me = this;
 ...
 * 让一个元素处于enable状态
 */
baidu.ui.behavior.statable.enable = function(element, group) {
    var me = this;
    element = element || me.getMain();
    if (me.getState(element.id, group)['disabled']) {
 ...
 * @param {string} eventHandler optional DOM事件处理函数.
 */
baidu.ui.behavior.statable.addState = function(state, eventName, eventHandler) {
    var me = this;
    me._allStates.push(state);
    if (eventName) {
        me._allEventsName.push(eventName);
 ...
 * @param {string} group optional    状态分组，同一组的相同状态会被加上相同的css.
 */
baidu.ui.behavior.statable.getState = function(elementId, group) {
    group = group ? group + '-' : '';
 ...
 * @param {string} group optional    状态分组，同一组的相同状态会被加上相同的css.
 */
baidu.ui.behavior.statable.setState = function(state, elementId, group) {
    group = group ? group + '-' : '';
    elementId = elementId ? elementId : this.getId();
    var me = this,
        stateId = group + elementId,
 ...
 * @param {string} group optional    状态分组，同一组的相同状态会被加上相同的css.
 */
baidu.ui.behavior.statable.removeState = function(state, elementId, group) {
    group = group ? group + '-' : '';
    elementId = elementId ? elementId : this.getId();
